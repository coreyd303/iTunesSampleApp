...
class RequestTests: QuickSpec {
    override func spec() {
        var sut: Request<GetMusicResponse>!

        beforeEach {
            sut = Request<GetMusicResponse>.stubGetTopAlbums()
        }

        describe("a Request") {
            it("should be an instance of Request") {
                expect(sut).to(beAnInstanceOf(Request<GetMusicResponse>.self))
            }
        }
        ...
        describe("execute") {
            ...
            var stubData: Data!
            var stubResponse: GetMusicResponse!
            var invokedCompletionCount: Int!
            var invokedCompletionParameters: (result: Result<GetMusicResponse>, ())?

            context("when result is .success") {
                beforeEach {
                    let encoder = JSONEncoder()
                    stubResponse = GetMusicResponse.stub()
